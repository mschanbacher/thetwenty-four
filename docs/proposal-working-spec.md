...
- **Register.** Monochromatic. Oxblood reserved for the hinge — the boundary line and the Boundary Games. No dates — timing belongs to the two-track calendar visual (site spec § 10, item 3b); do not double-encode it here.
...
- **BUILD STATE — built (this session), pending author's final sign-off.** Component shipped at `src/components/visuals/PromotionRelegation.tsx`; responsive toggle added to `globals.css` (`.promotion-relegation-wide/narrow`, same `@media (max-width: 639px)` breakpoint as the Field). Dual-variant, zero-JS, fully tokenized (0 hardcoded hex; 57 `var()` refs in wide output). Type-checks clean against the project config. Both variants' geometry verified byte-identical to the author-approved SVGs. As-built decisions that **amend the pre-build spec above**, all originating from the author's Illustrator passes:
  - **Labels changed by author, spec now follows.** The abstracted feeder's two promotion inputs are labeled **"Higher remaining seed" / "Lower remaining seed"** inside a "Semi-Final Losers" holding box, *not* "PP #3 / PP #4." The author's rationale: the descriptive labels state what the rank means rather than encoding it. This **supersedes the "Canonical labels PP #3 / PP #4" lock** in the Register bullet above and the § 2.5 label-lock generally, for this asset. Wiring is canonical (higher seed → CC #22 left game; lower → CC #21 right game); label *order* inside the box is Lower-above-Higher, chosen so the feeds do not cross.
  - **Stroke hierarchy as set by author (contested heavier than settled).** Spine 4px oxblood; contested/exchange paths (21, 22, both losers) 1.08px ink-muted; settled paths (23/24 down, winners up) 0.75px ink-muted. NB this **inverts** the certainty encoding proposed pre-build (originally: settled heavier). Retained as the author's deliberate choice. *Cut-line rhyme RESOLVED:* the Champions Field cut-line was raised 2px→4px (ChampionsField.tsx) to match this spine, so the oxblood boundary-line motif now matches on weight as well as color and role across both assets. Author confirmed on sight that Ch IV needed the heavier line; the Field follows to keep the system coherent.
...
- **Data source.** ~~Compressed-hypothetical records on the real 2025 team identities.~~ **SUPERSEDED (batch-2 build session).** Now: **the actual final 2025 CFP top-12 identities, in ranking order, seed = rank, with NO records column.** Final 2025 CFP ranking (regular season complete, Dec 7): 1 Indiana, 2 Ohio State, 3 Georgia, 4 Texas Tech, 5 Oregon, 6 Ole Miss, 7 Texas A&M, 8 Oklahoma, 9 Alabama, 10 Miami, 11 Notre Dame, 12 BYU. Carries a small "illustrative Champions Conference standings" label so it cannot read as a live CFP result (the real CFP applies conference-champion access rules our system does not — under our proposal it is purely top-12-by-ranking, which is the point). *Rationale for the reversal:* the author chose real identities over hypothetical because real programs make the mechanical-seeding argument (position → seed, full stop) undeniable in a way invented names cannot. Records were then dropped entirely — not compressed — because (a) the derivation argument is position→seed and needs no W-L, (b) real 2025 records contain undefeateds (Indiana 13-0) that would cross-reference awkwardly against the essay's balance claim at lines 80–84, and (c) dropping the records column is less ink, which the register prefers. The balance claim stays the essay's job, in prose, not the bracket's.
- **Visual form.** Side-by-side standings → bracket (horizontal, left→right, the way brackets conventionally read). The derivation is carried by two devices used lightly together: thin connective threads from each standings row to its bracket entry point (the "nothing comes from outside" argument) and a shared mono seed-number echo (the redundancy that makes it legible at a glance). Standard 2024 CFP shape: 12 teams, seeds 1–4 bye, 5–12 first round on the higher seed's campus, through the NY6 bowls to a mid-January final.
  - **BUILD RESOLUTION (batch-2, option B).** The two-device plan did not survive contact with geometry: bracket seed-order (1 / 8·9 / 4 / 5·12 / 3 / 6·11 / 2 / 7·10 top-to-bottom) differs from the standings order (1–12 straight down), so **threads between the two orderings necessarily cross** and read as a tangle — visually contradicting the "clean mapping" claim they were meant to support. Resolution: **drop the threads entirely; the mono seed-number echo alone carries the mapping.** The standings column stays a clean 1–12 with team names; the bracket shows **seed numbers only, no duplicated team names** (names live once, in the column). The reader connects "seed 5 in the column = seed 5 in the bracket" by the matching number — the lightest possible derivation device, and the more register-correct answer (Tufte: carry the mapping with a repeated token, not redundant ink). The bracket is structure; the column is the argument; the echo is the bridge.
  - **BUILD STATE — wide variant built and locked (batch-2), narrow pending.** Component at `src/components/visuals/ChampionsBracket.tsx` (new file, cousin of `ChampionsField` / `PromotionRelegation`); `.champions-bracket-wide/narrow` toggle added to `globals.css`. Wide geometry authored by the author in Illustrator — **even column pitch across all four rounds** (the author's improvement over the first build, which had a wide semifinal column); re-tokenized here (84 `var()` refs, 0 hardcoded hex), verified byte-identical to the author's file, type-checks clean. Round-label sequence: FIRST ROUND / QUARTERFINALS / SEMIFINALS / **CHAMPIONSHIP** (author's call — the label names the *game*, parallel to "Semifinals"; the winner *slot* still reads "Champion"). Cut-line 4px oxblood, below rank 12; oxblood coverage 0.076%. Data as amended above (real 2025 top 12, no records). **Narrow variant BUILT and locked** (720×700). Approach decided by testing two drafts against each other: a round-stack (rounds stacked as vertical bands) LOST the seed-echo derivation — the bracket bands drifted too far from the standings for the eye to pair matching seeds, and QF/SF slots became placeholder labels ("QF1 v QF2") carrying no information. Chosen instead: **option B — standings column and a compressed bracket SIDE BY SIDE**, a squeezed version of the wide rather than a reflow. Preserves BOTH the derivation (seed = rank, read across) AND the tournament structure. Verified legible at true 390px phone width. Author rebalanced spacing; both variants byte-identical to author files, dual component shipped. Design note: a horizontal bracket's narrow form is a *compression*, not a reorientation — the opposite lesson from 3c, whose already-vertical flow compressed trivially.
...
- **Essay edit.** Chapter IV bullets at essay lines 158–159 were corrected to the six-bye Promotion Playoff structure to match the CC spec; the prior summary did not close arithmetically. *Sub-item CLOSED:* the doubled-"the" typo is confirmed fixed in the repo (line 159 reads "join the top six seeds in the quarterfinals"); a doubled-word sweep of the full essay is clean. Essay dependency fully closed.

- **Site-spec § 10 drift to reconcile (documentation only).** Two key-visual items in `site-spec.md` § 10 were quietly reassigned during the graphics pass and never recorded, so § 10 read on its own still implies assets that don't exist as described:
  - **3c "Promotion Playoff + Boundary Games bracket"** was absorbed into Ch IV's *exchange* diagram, which deliberately does NOT draw the internal bracket. The internal bracket is now a **distinct, still-wanted asset** (the author's Ch IV bracket trials are its starting point). Recommend § 10 be back-annotated: 3c = standalone Promotion Playoff bracket; Ch IV = the exchange.

- **3c BUILD STATE — wide variant built and locked (batch-2), narrow pending.** Component at `src/components/visuals/PromotionPlayoffBracket.tsx` (new file); `.promotion-playoff-bracket-wide/narrow` toggle in `globals.css`. Wide 1000×1040 portrait, authored by the author in Illustrator (simplified from the first build — SF split-nodes and play-in dots removed, "QUARTERS" label), re-tokenized here (81 `var()` refs, 0 hardcoded hex), verified byte-identical, type-checks clean.
  - **Form = convergence to a split, not a champion.** Reads top-to-bottom; play-in → QF → SF, then the SF *splits*: winners promoted (out), losers to re-seed. Deliberately unlike Ch V's left-to-right expansion-to-apex — the Promotion Playoff has no championship game, so the bracket opens at the end rather than narrowing.
  - **Re-seed drawn honestly.** The two SF losers enter a gate ("re-seed by original standing / higher / lower"); two undifferentiated lines drop to a **single Boundary Games node** on the 4px oxblood membership line. Nothing downstream commits to which loser is higher — the conditional resolves inside the gate. This is the thing Ch IV could not draw in its cramped hinge, and the reason 3c exists as its own asset. (Author's simplification: single Boundary Games node rather than two, replacing the earlier two-game version.)
  - **Family motifs intact:** 4px oxblood boundary (shared with Ch IV spine + Champions Field cut-line), mono seeds (shared with Ch V), higher/lower descriptive labels (shared with Ch IV).
  - **Open (author's canvas, optional):** whether the gate needs a small "semifinal losers" label at its top input so the losers-in/re-seeded-out reading is unambiguous. Left to the author.
  - **Narrow variant BUILT and locked** (720×1200). 3c reflowed cheaply — it already flowed top-to-bottom, so horizontal compression alone did it (inverse of Ch IV, where the vertical exchange was the cheap narrow). Author cleaned the compressed spacing to match the wide (even play-in game widths, no overlaps). Both variants now byte-identical to author files, dual component shipped.
  - **3d "12-team CFP bracket"** became the Ch V bracket asset (already noted in Superseded).
  - **3b "Two-track calendar"** remains live and unbuilt, and is the reason Ch IV carries no dates. Its status (sixth `/proposal` visual, or dropped) is unresolved in the docs and worth an explicit decision.

- **Graphics batch 2 — decisions locked (planning session).** Building Ch V, then 3c, then 3b, as a deliberate family (three bracket/timeline-shaped assets that must differentiate rather than rhyme). Build order: **Ch V first, alone, locked before 3c begins** — Ch V sets the horizontal-bracket vocabulary that 3c deliberately diverges from.
  - **Differentiation axis.** Ch V = *expansion from a ranked list* (standings column left → bracket blooming right). 3c = *convergence to a re-seed* (a collapsing funnel echoing Ch IV's feeder, NOT a CFP-shaped advancement tree). Different silhouettes for a real reason: one derives a field, one re-seeds toward the boundary.
  - **3c label system LOCKED = "Higher / Lower remaining seed"** (same as Ch IV), NOT "PP #3 / PP #4." Reason is correctness, not just consistency: the 10-team bracket already carries entry seeds 1–10, so "PP #3" would collide with the bracket's seed #3 — a different team, since an entry seed need not survive to the Boundary Games. Descriptive labels name the relationship at the moment of re-seed and avoid the collision. This makes 3c and Ch IV visibly siblings on label treatment.
  - **3b confirmed IN SCOPE** as a real deliverable (author's call: the timing was thought through and should be shown). **Canonical data source = CC spec Part 4a** ("Calendar & Timing / Two Parallel Postseasons"), which already contains both a window-level overview table and a date-level detail table for both tracks. No data gap; it is a visualization problem, not a research one. 3b owns *timing* — the reason Ch IV, Ch V, and 3c all carry no dates.

- **3b BUILD STATE — BOTH variants built and locked (batch-2). Graphics batch 2 COMPLETE.** Component at `src/components/visuals/TwoTrackCalendar.tsx`; `.two-track-calendar-wide/narrow` toggle in `globals.css`. Wide 1000×820, narrow 720×730. Vertical time axis (late Nov → mid-Jan), CC events branching left, Promotion events right, mono dates down the center spine. Reflowed to narrow trivially (vertical axis = already a phone column, same as 3c). Both byte-identical to author-tuned files, fully tokenized (84/82 `var()` refs, 0 hex), type-checks clean.
  - **Register:** oxblood appears exactly once, at the Jan 4 Boundary Games crossing — the family hinge (Ch IV spine, 3c boundary, Champions Field cut-line). Faint full-width `paper-deep` band marks bowl season as the backdrop the whole postseason plays against (NOT a track event — full-width, drawn behind the tracks; author centered it on the Cotton Bowl row). Mono dates, serif event names, sans track headers.
  - **Argument made visible:** the two tracks run in parallel; the Promotion track goes silent after the Boundary Games while the CC track plays on alone to the championship — the asymmetry is legible in the ink, no annotation needed.
  - **New vocabulary:** first non-bracket asset — introduces the two-lane-with-center-spine timeline form. Sits in the family via oxblood/mono/serif but is structurally its own thing. Distinct from the two horizontal brackets; shares the vertical flow with 3c.
  - **Open (author's canvas, optional):** the Jan 4 matchup detail ("CC #21·22 v Promotion #3·4") restates what Ch IV/3c own — keep as a reminder or drop and let "Boundary Games" carry it.
