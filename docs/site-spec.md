...
- **Visuals live inside chapters.** Graphics candidates identified during drafting: Chapter IV (Promotion Playoff + Boundary Games bracket diagram), Chapter I (conference revenue over time or by conference), Chapter VII (Mississippi State revenue comparison, current vs proposal — the citation pass swapped Vanderbilt for Mississippi State because Mississippi State is public and FOIA-accessible, so the data dependency is now solvable without reporter contacts). Graphics pass is a dedicated future session after the citation and consistency passes complete.
...
- **Relationship to the landing page.** The landing page argues in compression; the essay argues at length. The essay assumes a reader who has seen the landing page but has not yet been convinced. Repetition of core claims is expected and appropriate. **Note for future work:** the landing copy was originally written at v0.6 before the essay existed; the landing-copy pass reconciled the two at v0.8 (*cleaner* → *better*, rivalry examples to canonical names, pullquote *"Staying becomes a strategy, not a sacrifice"* → *"Staying is a strategy"*, removal of *"not X, it is Y"* constructions to match the essay's pattern). See `docs/proposal-working-spec.md` § 2.3 for the full record.
...
- ~~**v0.7: Landing-copy tone drift.** The landing copy written in v0.6 is in a slightly different register than the essay settled into. Not a contradiction, but a seam. A targeted landing-copy pass will bring tone and terminology into alignment. See `docs/proposal-working-spec.md` § 2.3.~~ **Resolved in v0.8.** Landing-copy pass reconciled register and terminology (*cleaner* → *better*, rivalry examples to canonical names, *"not X, it is Y"* construction eliminated across the landing, pullquote tightened). Full record at `docs/proposal-working-spec.md` § 2.3.
...
2. ~~**Citation pass** on `/proposal`, chapter by chapter. The largest single work item. Chapter I and Chapter VII are the citation-heavy chapters; Chapter VII specifically depends on Vanderbilt revenue data that requires reporter or FOIA paths. Don't let Chapter VII block the other chapters' citation work — lock the others and hold Chapter VII open pending data. See `docs/proposal-working-spec.md` § 2.1.~~ **Complete in v0.8.** 18 inline links across 9 chapters. Chapter VII's revenue example swapped from Vanderbilt (private, not FOIA-accessible) to Mississippi State (public, FOIA-accessible), which closed the data dependency that had threatened to block the chapter.
3. ~~**Consistency pass** on `/proposal`. Runs cleanly after citations because citation-pass edits often touch prose. Fast relative to citations. See `docs/proposal-working-spec.md` § 2.2.~~ **Complete in v0.8.** 12 main-pass edits (chapter pointers, meta-commentary, capitalization); 13 sub-pass edits reducing the *"X is not A; it is B"* construction from 10+ instances to 2 retained.
4. ~~**Landing-copy rewrite** and **CC spec updates**. Can run in parallel with each other; together they form one short "adjacent documents" session. Both depend on the essay's voice being settled, which happens at the end of the consistency pass.~~ **Landing-copy pass complete in v0.8.** CC spec updates — terminology and Notre Dame / UConn sections — in progress as the final v0.8 item; glossary addition deferred to the `/spec` build session.
...
*End of spec v0.8. Revise directly in this file as decisions are made.*
